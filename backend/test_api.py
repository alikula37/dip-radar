...
def test_score_models_endpoint_exposes_feature_importance():
    ...
    assert "rule" in models and "learned_v5" in models
    rule = models["rule"]
...
def test_backtest_endpoint_replays_value_score_history():
    ...
    learned = client.get(
        "/api/backtest",
        params={"start": "2023-01-01", "score_model": "learned_v5", "min_score": 50},
    )
    assert learned.status_code == 200
    assert learned.json()["score_model"] == "learned_v5"
